...
import org.semux.api.response.ApiHandlerFailureResponse;
import org.semux.api.response.ApiHandlerSuccessResponse;
...
public class ApiHandlerImpl implements ApiHandler {
    ...
    public String service(String uri, Map<String, String> params, HttpHeaders headers) throws ApiHandlerException {
        ...
        try {
            switch (cmd) {
            case GET_INFO: {
                return success(new GetInfoResponse(kernel));
            }

            case GET_PEERS: {
                return success(kernel.getChannelManager().getActivePeers());
            }
            case ADD_NODE: {
            ...
            case GET_LATEST_BLOCK_NUMBER: {
                return success(kernel.getBlockchain().getLatestBlockNumber());
            }
            case GET_LATEST_BLOCK: {
                return success(kernel.getBlockchain().getLatestBlock());
            }
            case GET_BLOCK: {
                ...
                if (number != null) {
                    return success(kernel.getBlockchain().getBlock(Long.parseLong(number)));
                } else if (hash != null) {
                    return success(kernel.getBlockchain().getBlock(Hex.parse(hash)));
                } else {
                    return failure("Invalid parameter: number or hash can't be null");
                }
            }
            case GET_PENDING_TRANSACTIONS: {
                return success(kernel.getPendingManager().getTransactions());
            }
            ...
            case GET_TRANSACTION: {
                String hash = params.get("hash");
                if (hash != null) {
                    return success(kernel.getBlockchain().getTransaction(Hex.parse(hash)));
                } else {
            ...
            case GET_DELEGATE: {
                ...
                if (address != null) {
                    return success(new GetDelegateResponse(
                            kernel,
                            kernel.getBlockchain().getDelegateState().getDelegateByAddress(Hex.parse(address))));
                } else {
                    return failure("Invalid parameter: address can't be null");
                }
            }
            case GET_VALIDATORS: {
                return success(kernel.getBlockchain().getValidators().parallelStream()
                        .map(v -> Hex.PREF + v)
            ...
            case GET_DELEGATES: {
                return success(kernel.getBlockchain()
                        ...
                        .map(delegate -> new GetDelegateResponse(kernel, delegate))
                        .collect(Collectors.toList()));
            }
            case GET_VOTE: {
                ...
                if (voter != null && delegate != null) {
                    return success(
                            kernel.getBlockchain().getDelegateState().getVote(Hex.parse(voter),
                                    Hex.parse(delegate)));
                } else {
            ...
            case LIST_ACCOUNTS: {
                return success(kernel.getWallet().getAccounts().parallelStream()
    ...
    protected String success(Object result) throws ApiHandlerException {
        try {
            return new ObjectMapper().writeValueAsString(new ApiHandlerSuccessResponse(result));
    ...
    protected String success() throws ApiHandlerException {
        try {
            return new ObjectMapper().writeValueAsString(new ApiHandlerSuccessResponse(null));
        } catch (JsonProcessingException ex) {
            logger.error("failed to output success message", ex);
            throw new ApiHandlerException(INTERNAL_SERVER_ERROR);
        }
    ...
    protected String failure(String message) throws ApiHandlerException {
        try {
            return new ObjectMapper().writeValueAsString(new ApiHandlerFailureResponse(message));
        } catch (JsonProcessingException ex) {
