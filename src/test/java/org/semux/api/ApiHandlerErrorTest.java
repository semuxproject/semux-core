...
public class ApiHandlerErrorTest extends ApiHandlerTestBase {
    ...
    public static Collection<Object[]> data() {
        return Arrays.asList(new Object[][] { { "/add_node" }, { "/add_node?node=I_am_not_a_node" },
                { "/add_node?node=127.0.0.1:65536" }, { "/add_node?node=.com:5161" }, { "/add_to_blacklist" },
                { "/add_to_blacklist?ip=I_am_not_an_IP" }, { "/add_to_whitelist" },
                { "/add_to_whitelist?ip=I_am_not_an_IP" }, { "/get_block" }, { "/get_block?number=9999999999999999" },
                { "/get_account" }, { "/get_account?address=0xabc" }, { "/get_account?address=I_am_not_an_address" },
                { "/get_delegate" }, { "/get_delegate?address=" + Hex.encode(Bytes.random(20)) },
                { "/get_delegate?address=I_am_not_an_address" }, { "/get_account_transactions" },
                ...
                { String.format("/get_account_transactions?address=%s&from=%s&to=%s", randomHex(), "0",
                        "I_am_not_a_number") },
                { "/get_transaction" }, { String.format("/get_transaction?hash=%s", "I_am_not_a_hexadecimal_string") },
                { String.format("/get_transaction?hash=%s", randomHex()) }, { "/send_transaction" },
                { "/send_transaction?raw=I_am_not_a_hexadecimal_string" }, { "/get_vote" },
                ...
                { "/get_votes" }, { "/get_votes?delegate=I_am_not_hexadecimal_string" }, { "/transfer" },
