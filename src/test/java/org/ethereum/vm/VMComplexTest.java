...
public class VMComplexTest extends TestBase {
    ...
    public void test1() {
        ...
        byte[] contractAddrB = HexUtil.fromHexString(contractAddr);
        byte[] callerAddrB = HexUtil.fromHexString(callerAddr);
        byte[] codeB = HexUtil.fromHexString(code);

        ProgramInvoke pi = spy(invoke);
        when(pi.getOwnerAddress()).thenReturn(new DataWord(contractAddrB));
        ...
        repository.putStorageRow(contractAddrB, key1, value1);
    ...
    public void test2() {
        ...
        byte[] caller_addr_bytes = HexUtil.fromHexString(callerAddr);

        byte[] contractA_addr_bytes = HexUtil.fromHexString(contractA_addr);
        byte[] codeA = HexUtil.fromHexString(code_a);

        byte[] contractB_addr_bytes = HexUtil.fromHexString(contractB_addr);
        byte[] codeB = HexUtil.fromHexString(code_b);

        ProgramInvoke pi = spy(invoke);
        when(pi.getOwnerAddress()).thenReturn(new DataWord(contractB_addr_bytes));
        Repository repository = pi.getRepository();
        ...
        DataWord value_1 = repository.getStorageRow(contractA_addr_bytes, new DataWord(00));
        DataWord value_2 = repository.getStorageRow(contractA_addr_bytes, new DataWord(01));
    ...
    public void test3() {
        ...
        // Set contract into Database
        byte[] caller_addr_bytes = HexUtil.fromHexString("cd2a3d9f938e13cd947ec05abc7fe734df8dd826");

        byte[] contractA_addr_bytes = HexUtil.fromHexString("77045e71a7a2c50903d88e564cd72fab11e82051");
        byte[] contractB_addr_bytes = HexUtil.fromHexString("83c5541a6c8d2dbad642f385d8d06ca9b6c731ee");

        byte[] codeA = HexUtil.fromHexString("600b60005260166020526021604052602c6060526037608052604260a05260c06000f2");
        byte[] codeB = HexUtil.fromHexString(
    ...
    public void test4() {
        ...
        byte[] codeA = HexUtil.fromHexString("7f7f60c860005461012c602054000000000000" +
                "00000000000000000000000000006000547e60" +
                "005460206000f2000000000000000000000000" +
                "0000000000000000000000602054602960006064f0");

        ProgramInvoke pi = spy(invoke);
        when(pi.getOwnerAddress()).thenReturn(new DataWord(contractA_addr_bytes));
    ...
    public void test6() {
         ...
         * ]] (CALLSTATELESS 1000000 0x945304eb96065b2a98b57a48a06ae28d285a71b5 23 0 64
         * 64 0) }
         */

        // Set contract into Database
        byte[] caller_addr_bytes = HexUtil.fromHexString("cd1722f3947def4cf144679da39c4c32bdc35681");

        byte[] contractA_addr_bytes = HexUtil.fromHexString("945304eb96065b2a98b57a48a06ae28d285a71b5");
        byte[] contractB_addr_bytes = HexUtil.fromHexString("0f572e5295c57f15886f9b263e2f6d2d6c7b5ec6");

        byte[] codeA = HexUtil.fromHexString("60003554156009570060203560003555");
        byte[] codeB = HexUtil.fromHexString(
        ...
        ProgramInvoke pi = spy(invoke);
        when(pi.getOwnerAddress()).thenReturn(new DataWord(contractB_addr_bytes));
        when(pi.getGaslimit()).thenReturn(new DataWord(10000000000000l));
        ...
        DataWord storeValue1 = repository.getStorageRow(contractB_addr_bytes, new DataWord(00));
