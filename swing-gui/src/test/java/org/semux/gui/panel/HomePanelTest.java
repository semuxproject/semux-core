...
public class HomePanelTest extends AssertJSwingJUnitTestCase {
    ...
    protected void onSetUp() {
        ...
        Mockito.when(latestBlock.getNumber()).thenReturn(198L);
        Mockito.when(latestBlock.getTimestamp()).thenReturn(1527206400000L);
        Mockito.when(walletModel.getLatestBlock()).thenReturn(latestBlock);

        Key coinbase = new Key();
        Mockito.when(walletModel.getCoinbase()).thenReturn(coinbase);

        Mockito.when(walletModel.getStatus()).thenReturn(WalletModel.Status.NORMAL);
        Mockito.when(walletModel.getTotalAvailable()).thenReturn(Amount.ZERO);
        Mockito.when(walletModel.getTotalLocked()).thenReturn(Amount.ZERO);

        Mockito.when(walletModel.getAccounts()).thenReturn(Collections.emptyList());
    }
    ...
    public void testConsensusTable() {
        ...
        Mockito.when(walletModel.getValidatorDelegate(ArgumentMatchers.eq(0))).thenReturn(Optional.of(primaryValidatorDelegate));
        Mockito.when(walletModel.getValidatorDelegate(ArgumentMatchers.eq(1))).thenReturn(Optional.of(backupValidatorDelegate));
        ...
        Mockito.when(walletModel.getNextPrimaryValidatorDelegate()).thenReturn(Optional.of(nextPrimaryValidator));
        Mockito.when(walletModel.getNextValidatorSetUpdate()).thenReturn(Optional.of(200L));

        // mock kernel
        KernelMock kernelMock = Mockito.spy(kernelRule1.getKernel());
