...
public class DelegatesPanelTest extends AssertJSwingJUnitTestCase {
    ...
    public void testSelectDelegate() {
        Mockito.when(kernelMock.getPendingManager()).thenReturn(pendingManager);
    ...
    private void testVote(PendingManager.ProcessingResult mockResult) {
        ...
        Mockito.when(pendingManager.getNonce(ArgumentMatchers.any())).thenReturn(RandomUtils.nextLong());
        Mockito.when(pendingManager.addTransactionSync(ArgumentMatchers.any())).thenReturn(mockResult);
        Mockito.when(kernelMock.getPendingManager()).thenReturn(pendingManager);
        application = GuiActionRunner
                .execute(() -> new DelegatesPanelTestApplication(walletRule.walletModel, kernelMock));
        window = new FrameFixture(robot(), application);
